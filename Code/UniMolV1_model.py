...
from unimol_tools.weights import weight_download, WEIGHT_DIR
...
class UniMolModel(nn.Module):
    ...
    def forward(
        ...
        ###返回原子[PAD]掩码
        unimol_padding_mask = src_tokens.eq(self.padding_idx).float()
        if not unimol_padding_mask.any():
             unimol_padding_mask = torch.zeros_like(src_tokens).float()
        ...
        if return_repr:
            ...
            for tokens, coord in zip(src_tokens, src_coord):
                #print(f'Unimol1_tokens:{tokens}')
                filtered_tensor = tokens[
                    (tokens != 0) & (tokens != 1) & (tokens != 2)]  # filter out BOS(0), EOS(1), PAD(2)
                ...
                complete_tensors.append(tokens)
            ...
            if return_atomic_reprs:
                cls_atomic_reprs = []
                atomic_symbols = []
                complete_atomic_reprs = []
                complete_atomic_symbols = []
                ...
                # 统一原子数,过滤[CLS],[SEP]token,仅保留[PAD]
                for j in range(len(all_repr)):
                    padded_atomic_reprs = encoder_rep[j,:, :]
                    padded_atomic_symbol = []
                    for atomic_num in complete_tensors[j]:
                        padded_atomic_symbol.append(self.dictionary.symbols[atomic_num])
                    complete_atomic_symbols.append(padded_atomic_symbol)
                    complete_atomic_reprs.append(padded_atomic_reprs)
                #print("Atom_symbol:",complete_atomic_symbols)
                return {
                    'cls_repr': cls_repr,
                    'atomic_symbol': complete_atomic_symbols,
                    'atomic_coords': complete_coords,
                    'atomic_reprs': complete_atomic_reprs,
                    'atomic_mask': unimol_padding_mask
                }
            else:
