...
def get_smiles(mol):
    try:
        ...
        # 如果无法 Kekulé 化，不使用 Kekulé 化
        smiles = Chem.MolToSmiles(mol, kekuleSmiles=False,isomericSmiles=True)
    return Chem.MolToSmiles(mol, kekuleSmiles=True,isomericSmiles=True)
...
def get_clique_mol(mol, atoms):
    ...
    try:
        # 尝试 Kekulé 化
        Chem.Kekulize(mol)
        smiles = Chem.MolFragmentToSmiles(mol, atoms, kekuleSmiles=True,isomericSmiles=True)
    except Chem.KekulizeException:
        # 如果无法 Kekulé 化，不使用 Kekulé 化
        smiles = Chem.MolFragmentToSmiles(mol, atoms, kekuleSmiles=False,isomericSmiles=True)
